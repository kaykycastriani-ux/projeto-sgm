...
switch ($method) {
    ...
    case "PUT":
        ...
        if($conn->query($sql)==TRUE) {
            echo json_encode(["success" => true, "messagem" => "ambiente atualizado com sucesso!"]);
        }
}else{
    echo json_encode(["success"=>false, "messagem"=>"erro ao atualizar ambiente:". $conn->error]);
}
break;
case 'DELETE':
    $data= json_decode(file_get_contents("php://input"));
    if(!isset($data->id_ambinete)){
    ...
    default;
    echo json_encode(["success" => false, "mesagem" =>"metodo http não suporta."]);
    break;
?>